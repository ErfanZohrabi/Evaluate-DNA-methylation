rm(list=ls())
getwd()
setwd("C:/Users/asus/Desktop/project1")


### 1st Step; Load raw data with minfi and create and RGset
library(minfi)
baseDir <- ("C:/Users/asus/Desktop/Input_data_report/")
targets <- read.metharray.sheet(baseDir)
RGset <- read.metharray.exp(targets = targets)
save(RGset,file="RGset.RData")

### 2nd Step; Create a dataframe Red and Green for storing the red and green fluorescences
# to store flourescent intensity-->GetRed and GetGreen functions.
load("C:/Users/asus/Desktop/project1/RGset.RData")
...
load("C:/Users/asus/Desktop/Input_data_report/Illumina450Manifest_clean.RData") #load Illumina_manifest450_clean file

Illumina450Manifest_clean[Illumina450Manifest_clean$AddressA_ID=='40796508',]  #checking the address probe inside the manifest file
# infinium Type II, IlmnID = cg01311227 has both red and green
Illumina450Manifest_clean[Illumina450Manifest_clean$AddressB_ID=='40796508',] # No addressB_ID with this address because it is infinium II type
Illumina450Manifest_clean[Illumina450Manifest_clean$IlmnID=="cg12475994",]

Red[rownames(Red)=="40796508",]
Green[rownames(Green)=="40796508",]


### 3rd Step; CREATE THE OBJECT MSet.raw
MSet.raw <- preprocessRaw(RGset)
MSet.raw # it has 485512 probes and 8 samples
save(MSet.raw,file="MSet_raw.RData")


### 4th Step; Perform the following quality checks and provide a brief comment to each step
load("C:/Users/asus/Desktop/project1/MSet_raw.RData")
qc <- getQC(MSet.raw) # considering the median of methylation and unmethylation channels for each sample
#1
plotQC(qc)
#2
load("C:/Users/asus/Desktop/project1/MSet_raw.RData")
controlStripPlot(RGset, controls="NEGATIVE") # showing the intensity values for each type of controls probes in the samples
#3
detP <- detectionP(RGset) # calculating the detection p-value
failed <- detP>0.01
summary(failed)


### 6th Step; Getting raw M value from the MSet.raw object using getM(MSet.raw) --> values ranging from -infinite to + infinite
M <- getM(MSet.raw)
...
# Removing NA values if there is any
...
# density function for calculating each density distribution
...
# Now subsetting the beta matrix to obtain only rows that is in the first column of inf_I and inf_II
...
# preprocessNoob Normalization
preprocessNoob_data <- preprocessNoob(RGset)

# Getting the beta_processNoob matrix
beta_preprocessNoob <- getBeta(preprocessNoob_data)

# Dividing the obtained matrix according to typeI and typeII probes
...
beta_preprocessNoob_I_mean <- apply(beta_preprocessNoob_I, 1, mean)
beta_preprocessNoob_II_mean <- apply(beta_preprocessNoob_II, 1, mean)
beta_preprocessNoob_I_sd <- apply(beta_preprocessNoob_I, 1, sd)
beta_preprocessNoob_II_sd <- apply(beta_preprocessNoob_II, 1, sd)

# Calculate the density distribution for the 2 vectors of mean and SD values:
...
{plot(density(beta_preprocessNoob_I_mean, na.rm = T), col = "black", main = "Mean of normalized Beta values", xlim = c(0,1), ylim = c(0,4), xlab = "Beta mean", ylab = "Density")
  lines(density(beta_preprocessNoob_II_mean, na.rm = T), col = "#756bb1")
  legend(legend = c("TypeI", "TypeII"), col = c("black", "#756bb1"), "topright", lty = 1:1, bty = "n")}
{plot(density(beta_preprocessNoob_I_sd, na.rm = T), col = "black", main = "SD of normalize Beta values", xlim = c(0,0.5), ylim = c(0,30), xlab = "Beta SD", ylab = "Density")
  lines(density(beta_preprocessNoob_II_sd, na.rm = T), col = "#756bb1")
  legend(legend = c("TypeI", "TypeII"), col = c("black", "#756bb1"), "topright", lty = 1:1, bty = "n")}
boxplot(beta_preprocessNoob, ylim = c(0,1), main = "Normalized Beta values", las = 2, cex.axis = 0.4, col = colorbar)
...
# we create the t-test function and then applying it to the whole data
t_test_func <- function(x){
  t_test <- t.test(x ~ targets$Group)
  return (t_test$p.value)
}
p_value_t_test <- apply(beta_preprocessNoob[1:10000,], 1, t_test_func)
head(p_value_t_test, n = 10) # The first 10 entries


### 10th Step; MULTIPLE TEST CORRECTION
t_test_df <- data.frame(beta_preprocessNoob[1:10000,], p_value_t_test)
t_test_df <- t_test_df[order(t_test_df$p_value_t_test),] #  it contains sorted p-value of all probes
...
#Number of the probes with the p-value less than 0.05
dim(t_test_df_corrected[t_test_df_corrected$p_value_t_test <= 0.05,])

#Number of the probes that has passed the Bonferroni correction
dim(t_test_df_corrected[t_test_df_corrected$p_value_Bon <= 0.05,])

#Number of the probes that has passed the BH correction
